...
async function sendEmailFallback(orderData, user) {
  ...
  const mailOptions = {
    ...
    text: `New order received via fallback.\n\nOrder ID: ${orderData.id}\nUser: ${user.full_name}\nTotal: ${orderData.total_price}`
  };
  try {
    ...
    (err, user) => {
      ...
      if (!user) {
        db.get('SELECT COUNT(*) as count FROM users', [], (err2, row) => {
          ...
          const invitedBy = (start_param && /^[a-f0-9]{8}$/.test(start_param)) ? start_param : null;

          db.run(
            'INSERT INTO users (tg_id, username, full_name, role, referral_code, invited_by) VALUES (?, ?, ?, ?, ?, (SELECT tg_id FROM users WHERE referral_code = ?))',
